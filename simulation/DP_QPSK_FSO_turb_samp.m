%% 0. Environment Setup
clc; clear; close all;
%rng(2025);         % h_turb_samp = 1.4592
rng(256);          % h_turb_samp = 0.3147
...
I_turb = sample_logn(ch.n_turb);
% Aperture averaged
I_turb_av = sample_logn_av(ch.n_turb);
...
% Block fading was substituted by montecarlo 
...
for j = 1:ch.nframes
    h_turb_samp = sample_logn(1);           % one flat gain this frame (scalar)
    % Aperture averaged
    h_turb_samp_av = sample_logn_av(1);     % one flat gain this frame (scalar)
    % Apply FSO channel (amplitude scaling = sqrt of power gain)
    % h_ch = h_atm * h_tur * h_poin
    h_ch_samp = h_atm .* h_turb_samp;
    h_ch_samp_av = h_atm .* h_turb_samp_av;
    ...
    for i = 1:length(ch.EbN0_dB_vec)    
        ...
        % When the for is done we will do the craft constellations
        if j == ch.nframes
            rxmf_X_AWGN = upfirdn(tx_DP(:,1) + noise_X, rrc);
            rxmf_Y_AWGN = upfirdn(tx_DP(:,2) + noise_Y, rrc);
            rxmf_X_atm  = upfirdn(sqrt(h_atm) .* tx_DP(:,1) + noise_X, rrc);
            rxmf_Y_atm  = upfirdn(sqrt(h_atm) .* tx_DP(:,2) + noise_Y, rrc);
...
% Constellation AWGN + atmospheric effects + turbulence effects with
% aperture averaging
figure('Name',['Rx Symbols after Matched Filter - AWGN, atmospheric ' ...
'and turbulence effect w. aperture averaging']);

subplot(1,2,1);
plot(real(rx_symbols_X_av(1:2000)), imag(rx_symbols_X_av(1:2000)), 'bo', ...
    'MarkerFaceColor','b');
grid on; axis square;
xlabel('In-Phase (I)');
ylabel('Quadrature (Q)');
...
subplot(1,2,2);
plot(real(rx_symbols_Y_av(1:2000)), imag(rx_symbols_Y_av(1:2000)), 'ro', ...
    'MarkerFaceColor','r');
grid on; axis square;
xlabel('In-Phase (I)');
ylabel('Quadrature (Q)');
...
BER_table = table( ...
    ...
    'BER_atm_turb_theory_av','BER_X','BER_Y','BER_total'} );
...
ylabel('BER');
title('Gray-coded DP-QPSK with deterministic atmospheric attenuation, tubulence effects and AWGN');
%legend({'Theory AWGN','Theory atmosphere only', ...
%        'Theory atm + turb (point, $D=0$)', ...
%        sprintf('Theory atm + turb (aperture, $D=%.1f$\\,m)',scen.D_rx), ...
%        'X-Pol.', 'Y-Pol.', 'X-Pol. (aperture)', 'Y-Pol. (aperture)', ...
%        'Sim total (point)','Sim total (aperture)'}, ...
%        'Location','southwest','Interpreter','latex');
legend({'Theory AWGN','Theory atmosphere only', ...
    'Theory atm + turb (point, $D=0$)', ...
    sprintf('Theory atm + turb (aperture, $D=%.1f$\\,m)',scen.D_rx), ...
    'Sim total (point, $D=0$)','Sim total (aperture, $D=%.1f$\\,m)'}, ...
    'Location','southwest','Interpreter','latex');
ylim([1e-6 1]);
